    ...
    <main className="bg-white text-slate-900">
      <section className="border-b border-slate-200 bg-[#f7f9fc] pt-24 sm:pt-28">
        <div className="mx-auto max-w-7xl px-6 pb-16 sm:px-8 lg:px-10 lg:pb-20">
          ...
          <div className="mt-12 border-t border-slate-200 pt-5">
            <p className="mb-3 text-xs font-medium uppercase tracking-[0.16em] text-slate-400">Microsoft ecosystem</p>
            <div className="flex flex-wrap gap-x-7 gap-y-2 text-sm font-medium text-slate-600">{capabilities.map((item) => <span key={item}>{item}</span>)}</div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 sm:px-8 lg:px-10 lg:py-20">
        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
          <div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#2563eb]">The challenge</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.025em] text-slate-950 sm:text-4xl">Microsoft makes powerful tools available. Securing them properly is another matter.</h2></div>
          <div className="max-w-2xl text-lg leading-8 text-slate-600"><p>Microsoft 365 and Azure can become complicated quickly. Permissions grow, policies get missed, devices fall out of compliance and sensitive information can end up in places it should not.</p><p className="mt-5">My365Expert helps you understand what is happening in your environment, identify what matters and turn it into a practical improvement plan.</p></div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 sm:px-8 lg:px-10 lg:py-20"><div className="grid gap-10 lg:grid-cols-2 lg:items-center lg:gap-20"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#2563eb]">How we think</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.025em] text-slate-950 sm:text-4xl">Technology is only part of the problem.</h2></div><div><blockquote className="border-l-2 border-[#2563eb] pl-6 text-xl leading-8 text-slate-700 sm:text-2xl">Good security is not about turning on every Microsoft feature. It is about understanding your environment, knowing where the risks are and making sensible improvements without disrupting the business.</blockquote><p className="mt-5 pl-6 text-sm font-medium text-slate-500">My365Expert — Microsoft 365, Azure & Security Consulting</p></div></div></section>
